...
namespace ProjectTracking
{
  ...
  export class Project implements IProject
  {
    ...
    public static LoadProject(project: Project): void
    {
      ...
      Phase.LoadPhases(project.phases);
    ...
    public static BuildProjectSummaryList(projects: Array<Project>): void
    {
      ...
      for (let p of projects)
      {
        let phase_rows = Project.CreateNewGanttChartRows(p);
    ...
    public static CreateProjectSummaryRow(project: Project, phase_rows: Array<any>): HTMLTableRowElement
    {
      ...
      let newCurrentPhase = Phase.GetCurrentPhases(project, true, phase_rows);
      let phase_td = document.createElement("td");
    ...
    public static CreateProjectRow(project: Project): HTMLTableRowElement
    {
      ...
      //oldCurrentPhase.style.backgroundColor = "#00FFFF";
      let newCurrentPhase = Phase.GetCurrentPhases(project, false, []);
      let phase_td = document.createElement("td");
      //phase_td.appendChild(oldCurrentPhase);
      phase_td.appendChild(newCurrentPhase);
      tr.appendChild(phase_td); // get the current phase
    ...
    public static Save()
    {
      ...
      ProjectTracking.selected_project.phases = Phase.ReadPhases();
      if (!Phase.ValidatePhases(ProjectTracking.selected_project.phases))
      {
        Utilities.Toggle_Loading_Button("saveProject", false);
        return;
      }
    ...
    public static CreateNewGanttChartRows(project: Project): Array<any>
    {
      if (project.completed) return [];
      let phase_dates: Array<any> = [];
      ...
      //for (let i = 1; i < 6; i++)
      {
        ...
        //let start = <string>project["phase_" + i.toString() + "_start"];
        //let end = <string>project["phase_" + i.toString() + "_completion"];
        let name = phase.name.replace("Select Phase Name", "Phase Name not entered");
        let start = phase.started_on;
        let end = phase.completed_on !== null ? phase.completed_on : phase.estimated_completion;
        if (name.length > 0 && name !== "Ignore this Phase" && start !== null && end !== null)
        {
          phase_dates.push(
            {
              name: name,
